...
class MalmoEnvSpecial(gym.Env):
    ...
    def obs_to_vector(self,
        ...
        state_data = [
            self.state_map[block] if block in self.state_map else 0.0 for block in state_data_raw
        ]
        ...
        if use_entities:
            ...
            if add_inv:
                entity_data[0][entity_data.shape[1] // 2][entity_data.shape[2] //
                                                          2] = self.add_inventory(observation)
        ...
        if add_goal:
            state_data = np.concatenate(
                (np.ones((1, 9, 1)) * self.entity_map[self.goal], state_data), axis=2)
    ...
    def obs_to_ent_vector(self, observation, relevant_entities):
        ...
        for e in entities:
            ...
            entity_states[0][math.floor(relative_z)][math.floor(relative_x)] = float(
                self.entity_map[e["name"]])
    ...
    def build_arena(self):
        arena = ""
        arena += '<DrawCuboid x1="{}" y1="{}" z1="{}" x2="{}" y2="{}" z2="{}" type="bedrock" />'.format(
            -3, 203, -3, 3, 203, 3)
        arena += '<DrawCuboid x1="{}" y1="{}" z1="{}" x2="{}" y2="{}" z2="{}" type="bedrock" />'.format(
            -30, 203, -10, -3, 227, 10)
        arena += '<DrawCuboid x1="{}" y1="{}" z1="{}" x2="{}" y2="{}" z2="{}" type="bedrock" />'.format(
        ...
        arena += '<DrawCuboid x1="{}" y1="{}" z1="{}" x2="{}" y2="{}" z2="{}" type="bedrock" />'.format(
            -3, 203, 3, 3, 227, 10)
        arena += '<DrawCuboid x1="{}" y1="{}" z1="{}" x2="{}" y2="{}" z2="{}" type="air" />'.format(
            -2, 204, -2, 2, 270, 2)
    ...
    def get_mission_xml(self, mission_type):
        ...
        if mission_type == "pickaxe_stone":
            ...
            mission_xml = self.make_env_string(
                ...
                [
                    arena_xml,
                    '<DrawBlock x="{}" y="{}" z="{}" type="stone" />'.format(
                        random.randint(0, 4) - 2, 204, random.randint(1, 3) - 2),
                    '<DrawItem x="{}" y="{}" z="{}" type="diamond_pickaxe" />'.format(2, 206, 2)
                ])
            ...
            mission_xml = self.make_env_string(
                ...
                [
                    arena_xml,
                    '<DrawBlock x="{}" y="{}" z="{}" type="log" />'.format(
                        random.randint(0, 4) - 2, 204, random.randint(1, 3) - 2),
                    '<DrawItem x="{}" y="{}" z="{}" type="diamond_axe" />'.format(2, 206, 2)
                ])
        ...
        #     mission_xml = self.make_env_string(mission_type,[arena_xml,'<DrawBlock x="{}" y="{}" z="{}" type="clay" />'.format(random.randint(0,4)-2,204,random.randint(1,3)-2),'<DrawItem x="{}" y="{}" z="{}" type="diamond_shovel" />'.format(2,206,2) ])
            ...
            mission_xml = self.make_env_string(
                ...
                [
                    arena_xml,
                    '<DrawBlock x="{}" y="{}" z="{}" type="dirt" />'.format(
                        random.randint(0, 4) - 2, 204, random.randint(1, 3) - 2),
                    '<DrawItem x="{}" y="{}" z="{}" type="diamond_hoe" />'.format(2, 206, 2)
                ])
            ...
            mission_xml = self.make_env_string(
                ...
                [
                    arena_xml,
                    '<DrawBlock x="{}" y="{}" z="{}" type="water" />'.format(
                        random.randint(0, 4) - 2, 204, random.randint(1, 3) - 2),
                    '<DrawItem x="{}" y="{}" z="{}" type="bucket" />'.format(2, 206, 2)
                ])
    ...
    def __init__(self, mission_type, port, addr):
        ...
        #   print(mission_param)
        self.actions = ["movenorth", "movesouth", "movewest", "moveeast", "attack",
                        "use"]  #,"strafe 1","strafe -1"] #,"attack 1","attack 0"]
