...
def extract_cluster_names(text, labels):
    ...
    for tfidf_score,l in zip(X,labels):
            ...
            most_characteristic_words = sorted(all_words,key=lambda x: tfidf_score[words_to_index[x]],reverse=True)[:20]
...
def visualize_embeddings(text_to_embedding, reduce_fn, num_clusters,show=True):
    ...
    if show:
        ...
        for c,label in enumerate(set(labels)):
        	plt.scatter([i for i,l in zip(x,labels) if l == label],[i for i,l in zip(y,labels) if l == label],c=colors[c])
...
if sys.argv[1] == "VIZ":
    ...
    for label in set(labels):
    	print(process_text(" ".join([text_dict[t].encode("ascii", "ignore").decode() for  l,t in zip(labels,text) if l == label]),str(label)))
    	print("\n")
