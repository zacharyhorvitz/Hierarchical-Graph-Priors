...
class MalmoEnvSpecial(gym.Env):
	...
	def check_reached_goal(self):
		if self.current_mission == "pickaxe_stone":
			if self.object_2_index["cobblestone_item"] in self.inventory: return True
		elif self.current_mission == "axe_log":
			if self.object_2_index["log_item"] in self.inventory: return True
		elif self.current_mission == "hoe_farmland":
			if self.arena[self.player_y+1][self.player_x]  == self.object_2_index["farmland"]: return True
		elif self.current_mission == "bucket_water":
			if self.object_2_index["water_bucket_item"] in self.inventory: return True
